...
RUN set -x \
    ...
    && rm \
      ...
      /tmp/KEYS \
    && ln -s /opt/apacheds-*/bin/apacheds /usr/local/bin/ \
    && mv /var/lib/apacheds-* /var/lib/apacheds \
    && sed -ie 's/^INSTANCES_DIRECTORY=.*/INSTANCES_DIRECTORY="\/var\/lib\/apacheds"/g' /opt/apacheds-*/bin/apacheds \
